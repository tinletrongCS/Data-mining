...
const Tip = ({ active, payload }) => {
  ...
  return (
    <div className="custom-tooltip">
      <div className="label">Mốc: {payload[0].payload.bin}</div>
      <div className="val">{payload[0].value.toLocaleString()} người dùng</div>
    </div>
  )
}
        ...
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={preprocessComparison.after} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
            ...
            <YAxis tick={{ fill: '#94a3b8', fontSize: 10 }} axisLine={false} tickLine={false}
              tickFormatter={v => v >= 1000 ? `${(v/1000).toFixed(0)}K` : v} />
            <Tooltip content={<Tip />} cursor={{ fill: 'rgba(255,255,255,0.04)' }} />
            {/* Lưu ý dataKey ở đây là "c" tương ứng với JSON của PreprocessComparison */}
